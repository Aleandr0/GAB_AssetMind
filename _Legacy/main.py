...
class GABAssetMind:
    ...
    def _center_window(self):
        ...
        # Se le dimensioni non sono ancora state calcolate, usa quelle dal geometry
        if window_width < 100:  # Fallback per dimensioni non calcolate
            geometry = self.root.geometry()
            if 'x' in geometry:
                size_part = geometry.split('+')[0]
                window_width = int(size_part.split('x')[0])
                window_height = int(size_part.split('x')[1])
